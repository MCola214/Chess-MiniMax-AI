...
class Rook extends Piece {
	constructor(row, col, colour, hasMoved) {
		super(row, col, colour, "rook", hasMoved);
		this.value = 5;
	}

	copy() {
		const copy = JSON.parse(JSON.stringify(this));
		return new Rook(copy.row, copy.col, copy.colour, copy.hasMoved);
	}

	getMoveList(board) {
		const moveList = [];

		// check from current position to each set of vertical and horizontal line
		[[0, 1], [0, -1], [1, 0], [-1, 0]].forEach(tuple => {
			for (let i = 1; i < 8; i++) {
